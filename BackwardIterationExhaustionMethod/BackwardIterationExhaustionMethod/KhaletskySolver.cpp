...
KhaletskySolver::KhaletskySolver(const std::vector<std::vector<double>>& matrix,
	const std::vector<double>& f) : m_matrix(matrix), m_f(f) {}

bool KhaletskySolver::Solve(std::vector<double>& answer)
{
...
bool KhaletskySolver::createUpperAndLowerTriangularMatrix(std::vector<std::vector<double>>& UpperTriangularMatrix,
	std::vector<std::vector<double>>& LowerTriangularMatrix)
{
	...
	for (int i = 0; i < m_matrix.size(); ++i)
	{
		...
		for (int j = i + 1; j < m_matrix.size(); ++j)
		{
			...
			UpperTriangularMatrix[i][j] = (m_matrix[i][j] - sum) / LowerTriangularMatrix[i][i];
		}
	}
...
bool KhaletskySolver::solveUpperTriangularMatrix(const std::vector<std::vector<double>>& UpperTriangularMatrix,
...
bool KhaletskySolver::solveLowerTriangularMatrix(const std::vector<std::vector<double>>& LowerTriangularMatrix,
	const std::vector<double>& f, std::vector<double>& answer)
{
