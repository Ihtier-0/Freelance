...
int findIndexAbsMin(const std::vector<double>& vector)
{
    ...
    {
        if (std::abs(vector[i]) < std::abs(vector[indexAbsMin]))
        {
            indexAbsMin = i;
        }
    }
...
int findIndexSecondAbsMin(const std::vector<double>& vector)
{
    ...
    int indexAbsMin = 0;
    int indexSecondAbsMin = 1;

    if (std::abs(vector[indexAbsMin]) > std::abs(vector[indexSecondAbsMin]))
    {
        indexAbsMin = 1;
        indexSecondAbsMin = 0;
    }
    ...
    for (int i = 2; i < size; ++i)
    {
        if (std::abs(vector[i]) < std::abs(vector[indexAbsMin]))
        {
            indexSecondAbsMin = indexAbsMin;
            indexAbsMin = i;
        }
        else if (std::abs(vector[i]) < std::abs(vector[indexSecondAbsMin]))
        {
            indexSecondAbsMin = i;
        }
    }

    return indexSecondAbsMin;
}
...
int findIndexThirdAbsMin(const std::vector<double>& vector)
{
    ...
    int indexAbsMin = 0;
    int indexSecondAbsMin = 1;
    int indexThirdAbsMin = 2;

    for (int i = 3; i < size; ++i)
    {
        if (std::abs(vector[i]) < std::abs(vector[indexAbsMin]))
        {
            indexThirdAbsMin = indexSecondAbsMin;
            indexSecondAbsMin = indexAbsMin;
            indexAbsMin = i;
        }
        else if (std::abs(vector[i]) < std::abs(vector[indexSecondAbsMin]))
        ...
        else if (std::abs(vector[i]) < std::abs(vector[indexThirdAbsMin]))
        {
            indexThirdAbsMin = i;
        }
    }

    return indexThirdAbsMin;
}
